...
class TestParseArgs:
    ...
    def test_default_args(self):
        from epsionic.cli import parse_args
        args = parse_args([])
...
class TestMain:
    ...
    def test_setup_only_exits_early(self, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", ["epsionic", "--setup-only"])
        monkeypatch.setattr("epsionic.cli._auto_install", lambda **kw: None)
        rc = main()
        assert rc == 0
    ...
    def test_unknown_domain_returns_error(self, monkeypatch):
        ...
        monkeypatch.setattr("epsionic.cli.build_gateway", lambda cfg: mock_gw)
